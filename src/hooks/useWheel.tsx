import { WheelInfoOptions, useCompoundBody } from "@react-three/cannon";
import { useRef } from "react";

export const useWheels = (width: any, height: any, front: any, radius: any) => {
  const wheels = [
    useRef(null),
    useRef(null),
    useRef(null),
    useRef(null),
    useRef(null),
    useRef(null),
  ];

  const wheelInfo: WheelInfoOptions = {
    radius,
    directionLocal: [0, -1, 0],
    axleLocal: [-1, 0, 0],
    suspensionStiffness: 60,
    suspensionRestLength: 0.1,
    frictionSlip: 5,
    dampingRelaxation: 2.3,
    dampingCompression: 4.4,
    maxSuspensionForce: 100000,
    rollInfluence: 0.01,
    maxSuspensionTravel: 0.1,
    customSlidingRotationalSpeed: -30,
    useCustomSlidingRotationalSpeed: true,
  };

  const wheelInfos = [
    {
      ...wheelInfo,
      chassisConnectionPointLocal: [-width * 0.5, -height * 0.4, front - 0.5],
    },
    {
      ...wheelInfo,
      chassisConnectionPointLocal: [width * 0.5, -height * 0.4, front - 0.5],
    },
    {
      ...wheelInfo,
      chassisConnectionPointLocal: [width * 0.5, -height * 0.4, 0],
    },
    {
      ...wheelInfo,
      chassisConnectionPointLocal: [-width * 0.5, -height * 0.4, 0],
    },
    {
      ...wheelInfo,
      chassisConnectionPointLocal: [-width * 0.5, -height * 0.4, -front + 0.5],
    },
    {
      ...wheelInfo,
      chassisConnectionPointLocal: [width * 0.5, -height * 0.4, -front + 0.5],
    },
  ];

  const propsFunc = (): any => ({
    collisionFilterGroup: 0,
    mass: 1,
    shapes: [
      {
        args: [wheelInfo.radius, wheelInfo.radius, 0.015, 16],
        rotation: [0, 3, Math.PI / 2],
        type: "Cylinder",
      },
    ],
    type: "Kinematic",
  });

  useCompoundBody(propsFunc, wheels[0]);
  useCompoundBody(propsFunc, wheels[1]);
  useCompoundBody(propsFunc, wheels[2]);
  useCompoundBody(propsFunc, wheels[3]);
  useCompoundBody(propsFunc, wheels[4]);
  useCompoundBody(propsFunc, wheels[5]);

  return [wheels, wheelInfos];
};
